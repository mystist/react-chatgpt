...
export default function Index({ overlayMode }: any) {
  ...
  const scroll = useCallback((behavior = 'smooth' as ScrollBehavior) => {
    setTimeout(() => {
      if (divRef && divRef.current) divRef.current.scrollIntoView({ behavior })
    }, 70)
  }, [])
  ...
  useEffect(() => {
    if (prompt && prompt.system && !prompt.user) setChatMode('chat')
    if (prompt && prompt.user) setChatMode('task')

    if (sectionType) setIsShowPanel(true)
  }, [prompt, sectionType])

  useEffect(() => {
    if (talks.length > 0) scroll()
  }, [scroll, talks])
  ...
  return (
    <>
        ...
        <section className="w-full">
          <div className="flex h-full flex-col justify-between bg-white shadow sm:overflow-hidden sm:rounded-lg">
            <div className="px-4 py-6 sm:px-6">
              ...
              <div className="invisible" ref={divRef} />
            </div>
      ...
      <Transition.Root show={isShowClaim} as={Fragment}>
        <Dialog as="div" id="react-chatgpt" className="relative" onClose={setIsShowClaim} style={{ zIndex: 50 }}>
